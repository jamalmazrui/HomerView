...
- It reported that nothing oversized remained in the history when fifteen copies of the installer, twenty six megabytes each, were still there. Nothing was wrong with the check except the question it asked: it looked for a single file over GitHub's hundred megabyte limit, and no single copy came close. What matters is what a name costs in total, since a build output committed on every release accumulates. Counted that way the installer accounted for about three hundred and ninety megabytes, which is most of what a clone had to download. It is now counted by name across copies, and the closing line says what was actually checked rather than offering a bare reassurance.
- It also left three source modules out of the repository. The commit step used git add with the tracked-only switch, which was the fix for an earlier fault where the script committed its own log, and it went too far: a file git has never seen is not a tracked change, so linkTarget.py, mainContentFinder.py and homer/util.py were never added. Anyone cloning would have got an add-on that could not import three of its own modules. Untracked files are now sorted into those that belong, which are added, and those that do not, which are left alone.
- Adding files stages them, and the history rewrite refuses to run with anything staged, which is the same fault that appeared when untracking staged them. There is now one commit point covering both, so a third variant of it cannot appear.
- Tested against a repository built with fifteen distinct copies of a twenty six megabyte installer and three untracked source files: three hundred and ninety megabytes of history reduced to twenty kilobytes, the missing source added, and the development files untracked but still on disk.
...
- What it keeps is decided by what the setup script installs, what the build needs and what the repository tracks, rather than by a list that would drift from all three. If a file is already in the destination from a previous run, the newcomer gets the date on its name rather than overwriting something that may not have been looked at yet.
- The build now writes one add-on file rather than two. A copy named for the version was written beside HomerView.nvda-addon for release assets, and it was a mistake: two identical files with different names in one folder invites the wrong one being picked up, and the version is already in the manifest, which is what NVDA reads. cleanDir moves any left behind by an earlier build.
- Corrected two lines in .gitignore that did nothing. They used an exclamation mark to re-include executables and the built add-on, but an exclamation mark only re-includes what an earlier pattern excluded, and nothing excluded them. They were tracked because nothing stopped them, not because those lines said so. The comment now explains that, and HomerView_setup.exe is excluded, being a release asset rather than source and a large binary to commit on every release.
- Reviewed the setup script in full: every required source exists, the five files removed in 1.31.1 have not crept back, no directive is specified twice, no line begins with a bracket or a hash outside its proper place, and the licence the welcome page promises is the one installed. buildAll is now a required file rather than an optional one, since it carries the checks that run before Inno Setup.

## 1.32.1

- The user guide claimed that Jump to Main Content and Report Page Address had no key, when they are on J and Alt+A. The guide is generated from the code, and the generator read only the main gesture table; those two commands, and the address command's global key, are bound separately in the same file. It now reads every place a key is bound, and lists all of a command's keys shortest first, so the everyday key leads and the longer one follows.
- A guide that says a command is unassigned when it is on the letter J is worse than no guide, so buildAll.cmd now refuses to build if the guide contains that phrase.
- Added a section on the Invoke Script command, which had no documentation at all. It names the Stagehand idea it follows, lists every verb and its synonyms, explains how a target is named and how a value is given, gives a worked example, and says what Test, Help and OK each do. It also says plainly that the matching is HomerView's own, by role and accessible name, with nothing sent anywhere.
- Rewrote the guide around what a reader is trying to do rather than which file a command lives in: starting and finding things, getting around a page, finding text, reading and copying, asking about a page, acting on a page, and speech.
- On the Alternate Menu not opening: the log shows Alt+NVDA+F10 arriving and resolving correctly every time, then failing on a value that was nothing. That is the fault fixed in 1.27.0, and the log is from 1.26.4.

## 1.32.0

- A user reported that Google will not let him sign in to Gmail inside HomerView, saying the browser may not be secure. The report is accurate and the cause is structural.
- Google refuses to sign anyone in on a browser started with remote debugging. That switch is the whole basis of HomerView, since the connection is how it asks the browser what is on the page and tells it what to do, so it cannot be given up. The check is aimed at scripts driving a browser to take over accounts, which is a real problem and a fair thing to guard against; HomerView is not that, but the same switch enables both, so from Google's side the two are indistinguishable.
- One flag has been turned off: Chromium otherwise sets a marker on every page saying the browser is automated, and some sign-in pages read it and refuse. Turning that off is honest here, because this browser is driven by the person at the keyboard through their screen reader rather than by a script, which is what the marker was meant to detect and is not what it finds. It may help with other sites; it will not satisfy Google.
- Nothing further will be attempted. Defeating the check properly would mean making HomerView look like something it partly is, and the check exists for a good reason.
- What works is signing in before HomerView starts. A Google session established in an ordinary Edge window using the same profile is already there when HomerView opens it. The quick start now gives the steps, including allowing the profile to sign in at all, which it does not by default.
- Sites other than Google are mostly unaffected, and Microsoft accounts are not affected at all, which is why signing the profile in to a Microsoft account has always worked.
...
- The alternative of pressing Enter on the file in File Explorer is still mentioned, because it is genuinely shorter for anyone already in the folder.

## 1.26.3

- Reverted the installer to opening the add-on file through the shell, which is what it did originally and what worked, keeping only the flag that was missing.
- The change in 1.24.1 was a mistake. A user reported ShellExecuteEx failing with an access violation, and the shell was replaced with a direct call to nvda.exe. That was a speculative fix for a fault that could not be reproduced, and it broke the path that worked for everyone else: an installer running as administrator starts a child process as administrator, and an elevated NVDA cannot join the ordinary one already running.
- The likely explanation of both reports is the same missing flag rather than two different faults. Opening a file through the shell often reaches the running program in the user's own context, which is why it worked; but it is not guaranteed to, which would explain a failure on another machine. runasoriginaluser makes it certain, by running as the account that started Setup rather than the elevated one.
- So the mechanism is the original one with the flag it always needed, and nothing else changed. The NVDA lookup is kept, but only to answer whether NVDA is installed at all, so the finish page can give the manual route rather than letting the attempt fail with a number.
- The lesson is recorded here rather than only in a commit: a change made to fix a fault that cannot be reproduced should be the smallest one that could explain it, and replacing a working mechanism is not that.

## 1.26.2

- Fixed the installer failing to hand the add-on to NVDA, with a complaint about elevation even though NVDA was already running.
- The cause is structural rather than accidental. This installer requires administrator rights, because it writes to Program Files, and anything an elevated installer launches inherits that elevation. NVDA is running as the ordinary user, and a second copy started as administrator cannot join it: Windows treats the two as different sessions, so the add-on never reaches the NVDA the user is actually using.
- Both entries now carry Inno Setup's runasoriginaluser flag, which runs the program as the account that started Setup rather than as the elevated one. That is the flag written for exactly this case.
- On the recommended way generally: NV Access provides no command line switch for installing an add-on, and no supported way for an outside installer to do it. The two supported routes are NVDA's own Add-on Store and opening a .nvda-addon file, which NVDA registers itself to handle. Opening the file runs nvda.exe with the file as its argument, so calling nvda.exe that way is the same mechanism reached directly rather than through the file association, which is why it survives a missing or damaged association.
- The finish page now always gives the manual route through the Add-on store, not only when NVDA cannot be found. The automatic step can fail for reasons an installer cannot foresee, and this is the third such reason found in three releases.
- Both entries also no longer wait for NVDA to exit before Setup finishes, which it never should have done.
...
- If the release has no add-on attached, it says so and points at the releases page, rather than downloading an error page and handing that to NVDA. A downloaded file that does not begin with the two bytes every zip archive starts with is also refused now, for the same reason.
- The Alternate Menu now lists only commands that can do something. Before the browser is running, the commands that act on a page are left out, and what remains is launching it, the documents, the version check, the web lookups, opening a document, the log and the self test. Offering a command that cannot run means a reader chooses it and is told it does not apply, which is a wasted choice.
- In a page, the menu leaves out commands that need something this page does not have: saying or returning to a selection when there is none, and completing a selection that was never started.
- Found and fixed two functions that were being called but no longer existed, both lost in earlier edits of mine. The menu's readable names had been removed when the key naming was rewritten, which would have failed every time the menu opened; and edge.py used the log abbreviator without importing it, which would have failed when reopening the page the browser last had open. A check for names used but never defined now covers every module.
- Corrected four command keys still written in the old modifier order.
