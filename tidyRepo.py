...
Run it from C:\\HomerView with no arguments to see what it would do:

    python tidyRepo.py

Run it with --do-it to actually do it:

    python tidyRepo.py --do-it
...
def surveyWorkingTree(setNeeded):
    ...
    for sPath in lUntracked:
        ...
        bBelongs = isNeeded(sNormal, setNeeded) or sNormal.startswith("addon/")
...
def readVersionMessage():
    """A commit message naming the version, read from the manifest.
    ...
    """
    pathManifest = os.path.join(pathRoot, "addon", "manifest.ini")
    try:
        with open(pathManifest, encoding="utf-8-sig") as fileManifest:
            match = re.search(r'version\s*=\s*"([^"]+)"', fileManifest.read())
        if match:
            return f"HomerView {match.group(1)}"
    except OSError:
        pass
    return "Update HomerView"
...
def main():
    parser = argparse.ArgumentParser(description="Tidy the HomerView repository.")
    parser.add_argument("--do-it", action="store_true",
                        help="actually make the changes, rather than only describing them")
    ...
    if not os.path.isdir(os.path.join(pathRoot, ".git")):
        say("This is not a git repository. Run tidyRepo.py from C:\\HomerView.")
        return 1
    ...
    bNeedRewrite = bool(lTooBig) or bool(lBulky)
    say("=" * 68)
    say("PLAN")
    say("=" * 68)
    say("")
    ...
    if bNeedRewrite:
        ...
        iStep += 1
        lRemove = sorted({t[1] for t in lTooBig} | {s for _i, s in lBulky})
    ...
    if lStray or bNeedRewrite or lBelong:
        iStep += 1
        say(f"{iStep}. Commit.")
    ...
    if not bDoIt:
        say("This was a description only. Nothing has been changed.")
        say("")
        say("Run it again with --do-it to carry the plan out:")
        say("")
        say("    python tidyRepo.py --do-it")
        say("")
    ...
    if bNeedRewrite:
        sBackup = os.path.join(os.path.dirname(pathRoot),
                               f"HomerView_backup_{datetime.datetime.now():%Y%m%d_%H%M%S}")
    ...
    if bNeedRewrite:
        ...
        lNames = sorted({t[1] for t in lTooBig} | {s for _i, s in lBulky})
        for sName in lNames:
